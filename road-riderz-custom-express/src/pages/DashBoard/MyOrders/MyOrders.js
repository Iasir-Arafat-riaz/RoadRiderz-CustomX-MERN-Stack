...
const MyOrders = () => {
  ...
  const cancelOrder = (id) => {
    ...
    if (confirmation) {
      fetch(`https://mighty-caverns-68467.herokuapp.com/myOrder/${id}`, { method: "delete" })
        .then((res) => res.json())
        .then((data) => {
            console.log(data)
          ...
          {onlyMyOrder.map((orderInfo) => (
            <div className="my-order" key={orderInfo._id}>
              <div>
               ...
               <Link to={`/dashboard/MyOrders/${orderInfo._id}`}> <button className="btn btn-success mt-3 m-1" >Pay</button></Link>
              </div>
            </div>
          ))}
        </div>
